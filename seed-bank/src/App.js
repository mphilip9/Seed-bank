...
function App() {
  ...
  const searchZones = (zone, searchMonth = month) => {
    ...
    axios
      .get("/api", { params: { zone: zone, month: searchMonth } })
      .then((data) => {
  ...
  const retrieveUserData = (sub, zone) => {
    axios
      .get("/api/users", {
        params: { sub: sub, zone: zone, email: user.email },
      })
      .then((userData) => {
        userData.data.created_at = format(
          parseISO(userData.data.created_at),
          "MMMM d, yyyy"
        );
        ...
        <div className="App">
          ...
          <div>
            <Routes>
              ...
              <Route
                path="/myplants"
                element={<MyPlants profileData={profileData} />}
              />
...
const formatPlantData = (data) => {
  for (let i = 0; i < data.length; i++) {
    if (data[i].first_planting_start) {
      data[i].first_planting_start = format(
        parseISO(data[i].first_planting_start),
        "MMMM d"
      );
      data[i].first_planting_end = format(
        parseISO(data[i].first_planting_end),
        "MMMM d"
      );
    }
    if (data[i].second_planting_start) {
      data[i].second_planting_start = format(
        parseISO(data[i].second_planting_start),
        "MMMM d"
      );
      data[i].second_planting_end = format(
        parseISO(data[i].second_planting_end),
        "MMMM d"
      );
    }
  }
