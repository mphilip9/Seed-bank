...
exports.searchPlants = (req, res) => {
  ...
  if (req.query.zone.length > 3) {
    ...
    axios
      .get(`https://phzmapi.org/${zone}.json`)
      .then((data) => {
        ...
        console.log(newZone);
        db.query(
          `select * from plant_dates where ((first_planting_start, first_planting_end) overlaps
    ('2022-${currentMonth}-01'::DATE, '2022-${currentMonth}-28'::DATE) OR (second_planting_start, second_planting_end) overlaps
    ('2022-${currentMonth}-01'::DATE, '2022-${currentMonth}-28'::DATE)) AND  zone_start <= ${newZone} AND zone_end >= ${newZone};`
        )
          .then((result) => {
...
exports.getUserData = (req, res) => {
  ...
  db.query(findUsers, [user, zone, email])
    .then((success) => {
      db.query(getUsers, [user])
        .then((result) => {
